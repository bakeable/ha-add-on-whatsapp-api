      ...
      {!loading && currentEmpty && (
        <div className="text-center py-12 text-mushroom-text-muted">
          ...
          <p className="text-mushroom-text-secondary">
            No {tab === 'messages' ? 'messages' : tab === 'rules' ? 'rule executions' : 'events'} yet.
          </p>
    ...
    <div className="space-y-3">
      ...
      <div className="overflow-x-auto card p-0">
        <table className="min-w-full divide-y divide-mushroom-border">
          ...
          <tbody className="divide-y divide-mushroom-border">
            {events.map((evt) => (
              <tr key={evt.id} className="hover:bg-mushroom-card-hover transition-colors">
                <td className="px-4 py-3 whitespace-nowrap text-sm text-mushroom-text-muted">
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <div className="text-sm text-mushroom-text truncate max-w-[200px]" title={evt.chat_id || ''}>
                    {evt.chat_id || '-'}
                  </div>
                  {evt.sender_id && (
                ...
                <td className="px-4 py-3">
                  <div className="text-sm text-mushroom-text max-w-md truncate" title={evt.summary || ''}>
                    {evt.summary || <span className="text-mushroom-text-muted italic">(no summary)</span>}
                  </div>
                </td>
              </tr>
            ))}
